...
class FakeGameRepository : GameRepository {
    ...
    override fun getAllGameSessions(): Flow<List<GameSession>> = sessions.map {
        it.values.toList().sortedByDescending { s -> s.startedAt }
    }

    override fun getGameSessionById(id: Uuid): Flow<GameSession?> = sessions.map { it[id] }

    override fun getGameSessionsByStatus(status: GameStatus): Flow<List<GameSession>> = sessions.map { map ->
        map.values.filter { it.status == status }.sortedByDescending { it.startedAt }
    }

    override fun getInProgressGame(): Flow<GameSession?> = sessions.map { map ->
